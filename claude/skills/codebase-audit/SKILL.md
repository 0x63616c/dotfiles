...
7. **Log outcomes.** As findings are resolved or rejected, append an `## Outcomes` section to that audit file: `H1 — fixed <sha>` / `M3 — rejected: <reason>` / `H3 — deferred: <until-what>` / `H2 — false positive: <what the auditor got wrong>`. Record SHAs only AFTER the final push — a rebase during push rewrites them and leaves the audit trail pointing at dangling commits. False positives and rejections are the treasure — before the *next* audit of any repo, skim the latest audit files' Outcomes and fold recurring auditor mistakes back into audit-prompt.md or the repo's CLAUDE.md.
