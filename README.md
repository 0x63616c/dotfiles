...
| Path | What it does |
|---|---|
| `hammerspoon/init.lua` | Mic-watcher: auto pause/resume media around Wispr Flow dictation. Event-driven, not polled — `hs.audiodevice.watcher` plus a per-device `watcherCallback` on the default input fire when the mic's in-use state changes, and the global watcher re-attaches the per-device one when the default input itself changes (`dIn `/`dev#`). When an app grabs the mic (Wispr recording) it queries the current player via `media-control get --no-artwork` and sends an explicit `pause` only if something was actually playing; on release it sends `play` only if it was the one that paused. Explicit pause/play rather than blind toggles means a already-paused video is never accidentally started; `--no-artwork` is load-bearing, since large responses can block the `hs.task` pipe. A `session` counter discards async `media-control` callbacks that land after dictation has already ended. Pressing the physical play/pause media key mid-dictation sets a user-took-control flag and the auto-resume is skipped (an eventtap watches `systemDefined` key events; needs Accessibility). Known limits: resuming by clicking a player's UI mid-dictation isn't detected (only the media key is), and any mic-grabbing app (Zoom etc.) also triggers the pause. Also enables the `hs` CLI (`require("hs.ipc")` + `cliInstall("/opt/homebrew")` — `hs.ipc` is not loaded by default, and without it there's no terminal access to the running instance), and installs an `hs.pathwatcher` auto-reload: saving any `.lua` under the config dir calls `hs.reload()`, debounced through `hs.timer.delayed` (0.4s) because one save emits several FSEvents and each would otherwise rebuild the entire Lua state. Filters on the `.lua` extension only — never a path prefix, since FSEvents reports the resolved repo path rather than the `~/.hammerspoon` symlink. |
