...
| `skills/hammerspoon-spoons/` | Skill for finding/installing/configuring/authoring Spoons — `hs.loadSpoon`, `hs.spoons.use`, `SpoonInstall`, bundle anatomy, the catalog. Flags up front that **no Spoons are installed** (it's forward-looking, not describing current usage), and that because `~/.hammerspoon` is a symlink, installing one writes into this repo. |
...
| `hammerspoon/reload.lua` | Reloads the config when any `.lua` here is saved. An `hs.pathwatcher` on `hs.configdir` reloads on any `.lua` save, debounced through `hs.timer.delayed`; filters on extension, not path prefix, because `~/.hammerspoon` is a symlink and FSEvents reports the resolved repo path. Note a reload tears down `hs.ipc`, so an `hs -c` call racing a save will hang. |
| `hammerspoon/lib/` | Pure logic, deliberately free of any `hs.*` reference so it loads in a bare Lua interpreter — that constraint is the whole point, since `dictation.lua` builds eventtaps and watchers at load time and can therefore never be required outside Hammerspoon. `geometry.lua`: the ring path (bezier corners, notch detour, graceful fallback when the notch won't fit), the marquee offset, and `formatTime`/`subtitle` for the notch's position line. `chord.lua`: the Shift+Ctrl state machine, as a testable object — flag states and a clock in, "fire now" out. Not auto-loaded by `init.lua`, which only picks up top-level `.lua`. |
